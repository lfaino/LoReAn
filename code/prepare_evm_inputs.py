...
def convert_augustus(aug_file, wd):
    ...
    if os.path.isfile(out_file): 
        print ('Augustus GFF3 file existed already: ' + out_file + ' --- skipping\n')
        return out_file
...
def convert_genemark(genemark_file, wd):
    ...
    if os.path.isfile(out_file): 
        print ('GeneMark GFF3 file existed already: ' + out_file + ' --- skipping\n')
        return out_file
...
def evm_weight(evm_dir, weights_dic, evidences, pasa_name, gmap_name):
    ...
    evidence_dic = {'GeneMark.hmm':'ABINITIO_PREDICTION', 
            ...
            'AAT':'PROTEIN', pasa_name:'TRANSCRIPT', gmap_name: 'ABINITIO_PREDICTION'}
    software_links = {'genemark':'GeneMark.hmm', 
            'augustus':'Augustus', 'aat':'AAT', 'pasa':pasa_name, 'gmap' : gmap_name}
