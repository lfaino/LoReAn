...
def setting():
    ...
    parser.add_argument("-rp","--repeat_masked", nargs="?", default="", help="GFF or GFF3 or GTF or BED file containing repeats coordinates []",
                        metavar='GFF_file')
    parser.add_argument("-u","--update", nargs="?", default="", help="GFF or GFF3 or GTF or BED file containing repeats coordinates []",
                        metavar='GFF_file')
    parser.add_argument("-m","--max_long_read", nargs="?", default=20000,
    ...
    parser.add_argument("-n","--prefix_gene", nargs="?", default="species", help="Prefix to add to the final Gff3 gene name [specie]")
    parser.add_argument("-w","--working_dir", nargs="?", default="annotation", help="Working directory (will create if not present) [./]")
    ...
    parser.add_argument("-cw","--augustus_weigth",  default="1", help="Weight assigned to AUGUSTUS evidence for EVM [1]",
                        metavar='N')
    parser.add_argument("-uw", "--update_weigth", default="1", help="Weight assigned to GFF3 file to update for EVM [1]",
                        metavar='N')
    parser.add_argument("-gw","--genemark_weigth", nargs="?", default="1", help="Weight assigned to GENEMARK evidence for EVM [1]",
                        metavar='N')
    parser.add_argument("-tw","--trinity_weigth", nargs="?", default="1", help="Weight assigned to Trinity mapped with GMAP evidence for EVM [1]",
                        metavar='N')
    ...
    parser.add_argument("-ee", "--end_exon", nargs="?", default="20", help="Minimal length for end exon with GMAP [20]",
                        metavar='N')
    parser.add_argument("-cme","--cluster_min_evidence", nargs="?", default="5", help="Minimal evidence needed to form a cluster [5]",
                        metavar='N')
