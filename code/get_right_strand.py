...
def removeDiscrepancy(gff, evmFile):
    badName = []
    errorFile = gff + ".bedtools_err.log"
    ...
    for evm in listgene:
        ...
        for i in db1.children(evm, featuretype='exon', order_by='start'):
            gff_out.write_rec(i)
    bedtools_call = subprocess.Popen(['bedtools', 'intersect', '-v', '-a', evmFile, '-b', gff], stdout=subprocess.PIPE, stderr=file(errorFile, 'w'))
    evm_mRNA = []
...
def removeOverlap(gff, overlap):
    ...
    bedmerge = ['bedtools', 'merge', '-d', str(overlap), '-s', '-o', 'count,distinct','-c','9,9' ]
