            ...
            if 'output' in direc:
                ...
                if fname_exists:
                    ...
                    for line in support:
                        ...
                        line = line.split('\t')
                        if line[2] in name_list:
                            coverage.write(line[2] + '\t' + line[6] +  '\t' +  line[7]+'\n')
                    coverage.close()
                    ...
                    for line in bed:
                        ...
                        info = line.split('\t')
                        #print info[0]
                        if unitigDict.has_key(info[0]) and int(info[3]) > int(int(unitigDict[info[0]])/20) and rangeDict.has_key(info[0]):
                             rangeDict[info[0]].append(info[2])
                            
                        elif unitigDict.has_key(info[0]) and int(info[3]) > int(int(unitigDict[info[0]])/20):
                    ...
                    for record in outFasta:
                        name_seq = record.id
                        if finalDict.has_key(name_seq):
                            location = finalDict[name_seq]
                            nucleotide = record.seq
                            newNucl = nucleotide[int(finalDict[name_seq][0]):int(finalDict[name_seq][1])]
                            sequence.write('>' + str(name_seq) + '\n' + str(newNucl) +  '\n' )
                        else:
