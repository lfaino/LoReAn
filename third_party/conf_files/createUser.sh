...
name=$1
uid=$2
adduser --disabled-password --uid $uid --gecos '' $name && adduser $name sudo && echo '%sudo ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers &> log.txt
...
usermod -d /var/lib/mysql/ mysql &> log.txt
/etc/init.d/mysql start &> log.txt
mysql --user="root" --password="lorean" --execute="set global sql_mode='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';" &> log.txt
