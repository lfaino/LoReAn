...
main: {
	...
	unless (-d "$genomeBaseDir/$genomeDir") {
        ...
        my $cmd = "gmap_build -D $cwd -T $genomeBaseDir -d $genomeDir -k 13 $genome >&2";
		&process_cmd($cmd);
	}
	...
	my $cmd = "gmap -D $cwd -d $genomeDir $transcriptDB -f $format -n $num_gmap_top_hits -x 50 -t $CPU -B 5 ";
	if ($max_intron) {
